...
const (
	...
	URN_WANTimnetLinkConfig_1    = "urn:schemas-upnp-org:service:WANTimnetLinkConfig:1"
...
//
// * NewLinkType: allowed values: Timnet
...
// Return values:
//
// * NewWANAccessType: allowed values: DSL, POTS, Cable, Timnet
//
...
// WANTimnetLinkConfig1 is a client for UPnP SOAP service with URN "urn:schemas-upnp-org:service:WANTimnetLinkConfig:1". See
// goupnp.ServiceClient, which contains RootDevice and Service attributes which
// are provided for informational value.
type WANTimnetLinkConfig1 struct {
	goupnp.ServiceClient
}

// NewWANTimnetLinkConfig1Clients discovers instances of the service on the network,
// and returns clients to any that are found. errors will contain an error for
// any devices that replied but which could not be queried, and err will be set
// if the discovery process failed outright.
//
// This is a typical entry calling point into this package.
func NewWANTimnetLinkConfig1Clients() (clients []*WANTimnetLinkConfig1, errors []error, err error) {
	var genericClients []goupnp.ServiceClient
	if genericClients, errors, err = goupnp.NewServiceClients(URN_WANTimnetLinkConfig_1); err != nil {
		return
	}
	clients = newWANTimnetLinkConfig1ClientsFromGenericClients(genericClients)
	return
}

// NewWANTimnetLinkConfig1ClientsByURL discovers instances of the service at the given
// URL, and returns clients to any that are found. An error is returned if
// there was an error probing the service.
//
// This is a typical entry calling point into this package when reusing an
// previously discovered service URL.
func NewWANTimnetLinkConfig1ClientsByURL(loc *url.URL) ([]*WANTimnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsByURL(loc, URN_WANTimnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANTimnetLinkConfig1ClientsFromGenericClients(genericClients), nil
}

// NewWANTimnetLinkConfig1ClientsFromRootDevice discovers instances of the service in
// a given root device, and returns clients to any that are found. An error is
// returned if there was not at least one instance of the service within the
// device. The location parameter is simply assigned to the Location attribute
// of the wrapped ServiceClient(s).
...
// This is a typical entry calling point into this package when reusing an
// previously discovered root device.
func NewWANTimnetLinkConfig1ClientsFromRootDevice(rootDevice *goupnp.RootDevice, loc *url.URL) ([]*WANTimnetLinkConfig1, error) {
	genericClients, err := goupnp.NewServiceClientsFromRootDevice(rootDevice, loc, URN_WANTimnetLinkConfig_1)
	if err != nil {
		return nil, err
	}
	return newWANTimnetLinkConfig1ClientsFromGenericClients(genericClients), nil
...
func newWANTimnetLinkConfig1ClientsFromGenericClients(genericClients []goupnp.ServiceClient) []*WANTimnetLinkConfig1 {
	clients := make([]*WANTimnetLinkConfig1, len(genericClients))
	for i := range genericClients {
		clients[i] = &WANTimnetLinkConfig1{genericClients[i]}
	}
	return clients
}
...
// Return values:
//
// * NewTimnetLinkStatus: allowed values: Up, Down
func (client *WANTimnetLinkConfig1) GetTimnetLinkStatus() (NewTimnetLinkStatus string, err error) {
	// Request structure.
	request := interface{}(nil)
	// BEGIN Marshal arguments into request.
	...
	// Response structure.
	response := &struct {
		NewTimnetLinkStatus string
	}{}

	// Perform the SOAP call.
	if err = client.SOAPClient.PerformAction(URN_WANTimnetLinkConfig_1, "GetTimnetLinkStatus", request, response); err != nil {
		return
	}

	// BEGIN Unmarshal arguments from response.

	if NewTimnetLinkStatus, err = soap.UnmarshalString(response.NewTimnetLinkStatus); err != nil {
		return
	}
	// END Unmarshal arguments from response.
	return
