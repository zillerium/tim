...
// Reference: https://github.com/tim-coin/wiki/wiki/thash-DAG
#define DAG_MUTABLE_NAME_MAX_SIZE (6 + 10 + 1 + 16 + 1)
/// Possible return values of @see thash_io_prepare
enum thash_io_rc {
	thash_IO_FAIL = 0,           ///< There has been an IO failure
	thash_IO_MEMO_SIZE_MISMATCH, ///< DAG with revision/hash match, but file size was wrong.
	thash_IO_MEMO_MISMATCH,      ///< The DAG file did not exist or there was revision/hash mismatch
	thash_IO_MEMO_MATCH,         ///< DAG file existed and revision/hash matched. No need to do anything
};
...
/**
 * Logs a critical error in important parts of thash. Should mostly help
 * figure out what kind of problem (I/O, memory e.t.c.) causes a NULL
 * thash_full_t
 */
#ifdef thash_PRINT_CRITICAL_OUTPUT
#define thash_CRITICAL(...)							\
	do													\
	{													\
		printf("thash CRITICAL ERROR: "__VA_ARGS__);	\
		printf("\n");									\
		fflush(stdout);									\
	} while (0)
#else
...
/**
 * Prepares io for thash
 *
 * Create the DAG directory and the DAG file if they don't exist.
 *
 * @param[in] dirname        A null terminated c-string of the path of the thash
 ...
 *                           https://github.com/tim-coin/wiki/wiki/thash-DAG
 ...
 *                           already exists
 * @return                   For possible return values @see enum thash_io_rc
 */
 ...
 */
FILE* thash_fopen(char const* file_name, char const* mode);
 ...
 *                         error returns NULL
 */
char* thash_strncat(char* dest, size_t dest_size, char const* src, size_t count);
 ...
 *                       existed
 */
bool thash_mkdir(char const* dirname);
 ...
 */
bool thash_file_size(FILE* f, size_t* ret_size);
 ...
 *
 * The spec defining this directory is here: https://github.com/tim-coin/wiki/wiki/thash-DAG
 *
 ...
 */
bool thash_get_default_dirname(char* strbuf, size_t buffsize);

static inline bool thash_io_mutable_name(
	uint32_t revision,
