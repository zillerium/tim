...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", timd.LightScryptN, timd.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.tim", new NodeConfig());
			node.start();
			...
			timClient ec = node.gettimClient();
	...
	// signatures too. Regression test for tim issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altotimder
	...
	workspace, err := ioutil.TempDir("", "timd-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/tim/timdtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for timd and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.tim", "github.com/tim-coin/tim/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "timd.aar"), "timd.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "tim", "timdtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
