...
// Package timstats implements the network stats reporting service.
package timstats
...
// Service implements an tim netstats reporting daemon that pushes local
	...
	for {
		...
		if len(msg["emit"]) == 2 && command == "history" {
			...
			if !ok {
				...
				s.histCh <- nil
				continue // timstats sometime sends invalid history requests, ignore those
			}
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols["tim"]; info != nil {
		network = fmt.Sprintf("%d", info.(*eth.TimNodeInfo).Network)
		protocol = fmt.Sprintf("eth/%d", eth.ProtocolVersions[0])
	} else {
		network = fmt.Sprintf("%d", infos.Protocols["les"].(*eth.TimNodeInfo).Network)
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the timstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to timstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to timstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to timstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to timstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to timstats")
