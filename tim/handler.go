...
import (
	...
	"github.com/tim-coin/tim/eth/fetcher"
...
func NewProtocolManager(config *params.ChainConfig, mode downloader.SyncMode, networkId uint64, mux *event.TypeMux, txpool txPool, engine consensus.Engine, blockchain *core.BlockChain, chaindb timdb.Database) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) removePeer(id string) {
	...
	log.Debug("Removing tim peer", "peer", id)

	// Unregister the peer from the downloader and tim peer set
...
func (pm *ProtocolManager) Stop() {
	log.Info("Stopping tim protocol")
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("tim peer connected", "name", p.Name())

	// Execute the tim handshake
	td, head, genesis := pm.blockchain.Status()
	if err := p.Handshake(pm.networkId, td, head, genesis); err != nil {
		p.Log().Debug("tim handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("tim peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("tim message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
		...
		for !unknown && len(headers) < int(query.Amount) && bytes < softResponseLimit && len(headers) < downloader.MaxHeaderFetch {
			...
			case query.Origin.Hash != (common.Hash{}) && !query.Reverse:
				...
				if next <= current {
					...
					if header := pm.blockchain.timdeaderByNumber(next); header != nil {
	...
	case msg.Code == BlockHeadersMsg:
		...
		if len(headers) == 0 && p.forkDrop != nil {
			...
			if daoHeader := pm.blockchain.timdeaderByNumber(pm.chainconfig.DAOForkBlock.Uint64()); daoHeader != nil {
...
// EthNodeInfo represents a short summary of the tim sub-protocol metadata known
// about the host peer.
type EthNodeInfo struct {
	Network    uint64      `json:"network"`    // tim network ID (1=Frontier, 2=Morden, Ropsten=3)
