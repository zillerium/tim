...
const (
	// bloomServiceThreads is the number of goroutines used globally by an tim
...
func (eth *tim) startBloomHandlers() {
...
// for the tim header bloom filters, permitting blazing fast filtering.
type BloomIndexer struct {
	size uint64 // section size to generate bloombits for

	db  timdb.Database       // database instance to write index data and metadata into
...
func NewBloomIndexer(db timdb.Database, size uint64) *core.ChainIndexer {
	backend := &BloomIndexer{
		db:   db,
		size: size,
	}
	table := timdb.NewTable(db, string(core.BloomBitsIndexPrefix))
