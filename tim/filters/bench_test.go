...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/timd/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := timdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
		b.Fatalf("error opening database at %v: %v", benchDataDir, err)
	}
	head := core.timdeadBlockHash(db)
	if head == (common.Hash{}) {
	...
	for sectionIdx := uint64(0); sectionIdx < cnt; sectionIdx++ {
		...
		for i := sectionIdx * sectionSize; i < (sectionIdx+1)*sectionSize; i++ {
			...
			header = core.Timheader(db, hash, i)
			if header == nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = timdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db timdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*timdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db timdb.Database) {
