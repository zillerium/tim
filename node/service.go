...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (timdb.Database, error) {
	if ctx.config.DataDir == "" {
		return timdb.NewMemDatabase()
	}
	db, err := timdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
