...
func (n *Node) OpenDatabase(name string, cache, handles int) (timdb.Database, error) {
	if n.config.DataDir == "" {
		return timdb.NewMemDatabase()
	}
	return timdb.NewLDBDatabase(n.config.resolvePath(name), cache, handles)
}
