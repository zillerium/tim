...
import (
	...
	"github.com/tim-coin/tim/accounts/usbwallet"
...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of timd is "timd". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "timd", keep that.
	if name == "timd" || name == "timd-testnet" {
		name = "timd"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "timd" instances.
var isOldtimdResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by timd 1.4 are used if they exist.
	if c.name() == "timd" && isOldtimdResource[path] {
		oldpath := ""
		if c.Name == "timd" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	default:
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "tim-keystore")
		ephemeral = keydir
	}
	if err != nil {
		return nil, "", err
