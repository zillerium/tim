...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and tim address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an tim transaction after having the user validate the parameters
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "tim app in browser mode", w.failure
	}
	if w.offline() {
		return "tim app offline", w.failure
	}
	return fmt.Sprintf("tim app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whether the wallet and the tim app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// tim app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the tim app's version, will fail prior to v1.0.2
...
// and returning the tim address located on that derivation path.
...
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the tim app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the tim wallet app running
...
// ledgerDerive retrieves the currently active tim address from a Ledger
...
//   tim address length | 1 byte
//   tim address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the tim hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an tim address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the tim signature and do a sanity validation
	if len(reply) != 65 {
