...
func loadConfig(file string, cfg *timdConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func makeFullNode(ctx *cli.Context) *node.Node {
	...
	// Add the tim Stats daemon if requested.
	if cfg.timstats.URL != "" {
		utils.RegistertimstatsService(stack, cfg.timstats.URL)
	}
	...
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		...
		utils.Fatalf("Failed to register the timd release oracle service: %v", err)
	}
	return stack
}
