...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"timstats", "explorer", "wallet", "faucet"} {
	...
	// If we have timstats running, ask whether to make the secret public or not
	var timstats bool
	if w.conf.timstats != "" {
		fmt.Println()
		fmt.Println("Include timstats secret on dashboard (y/n)? (default = yes)")
		timstats = w.readDefaultString("y") == "y"
	}
	// Try to deploy the dashboard container on the host
	if out, err := deployDashboard(client, w.network, infos.port, infos.host, listing, &w.conf, timstats); err != nil {
