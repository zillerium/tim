...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.timstats == "" {
		log.Error("No timstats server configured")
		return
	}
	...
	if infos.timstats == "" {
		...
		infos.timstats = w.readString() + ":" + w.conf.timstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.timstats)
		infos.timstats = w.readDefaultString(infos.timstats) + ":" + w.conf.timstats
	}
	...
	if out, err := deployNode(client, w.network, w.conf.bootFull, w.conf.bootLight, infos); err != nil {
		log.Error("Failed to deploy tim node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
