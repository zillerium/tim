...
func (w *wizard) networkStats(tips bool) {
	...
	for server, pubkey := range w.conf.Servers {
		...
		if infos, err := checktimstats(client, w.network); err != nil {
			...
			services["timstats"] = infos.String()
			protips.timstats = infos.config
		}
	...
	if protips.timstats != "" {
		w.conf.timstats = protips.timstats
	}
...
func (p *protips) print(network string) {
	...
	fullinit, lightinit := "", ""
	if p.genesis != "" {
		fullinit = fmt.Sprintf("timd --datadir=$HOME/.%s init %s.json && ", network, network)
		lightinit = fmt.Sprintf("timd --datadir=$HOME/.%s --light init %s.json && ", network, network)
	}
	// If an timstats server is available, add the timstats flag
	statsflag := ""
	if p.timstats != "" {
		if strings.Contains(p.timstats, " ") {
			statsflag = fmt.Sprintf(` --timstats="yournode:%s"`, p.timstats)
		} else {
			statsflag = fmt.Sprintf(` --timstats=yournode:%s`, p.timstats)
		}
	}
	...
	tips = append(tips, fmt.Sprintf("%stimd --networkid=%d --datadir=$HOME/.%s --cache=1024%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a full node with recent data only")
	tips = append(tips, fmt.Sprintf("%stimd --networkid=%d --datadir=$HOME/.%s --cache=512 --fast%s%s", fullinit, p.network, network, statsflag, bootflagFull))

	tasks = append(tasks, "Run a light node with on demand retrievals")
	tips = append(tips, fmt.Sprintf("%stimd --networkid=%d --datadir=$HOME/.%s --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))

	tasks = append(tasks, "Run an embedded node with constrained memory")
	tips = append(tips, fmt.Sprintf("%stimd --networkid=%d --datadir=$HOME/.%s --cache=32 --light%s%s", lightinit, p.network, network, statsflag, bootflagLight))
