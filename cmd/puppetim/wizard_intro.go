...
// makeWizard creates and returns a new puppetim wizard.
...
// setting up a new or managing an existing tim private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to puppetim, your tim private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new tim network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and timstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Puppetim uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering tim network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".puppetim", w.network)
