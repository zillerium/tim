...
RUN \
  apk add --update git go make gcc musl-dev ca-certificates linux-headers                             && \
	mkdir -p $GOPATH/src/github.com/tim-coin                                                            && \
	(cd $GOPATH/src/github.com/tim-coin && git clone --depth=1 https://github.com/tim-coin/tim) && \
  go build -v github.com/tim-coin/tim/cmd/faucet                                              && \
...
func (info *faucetInfos) String() string {
	return fmt.Sprintf("host=%s, api=%d, eth=%d, amount=%d, minutes=%d, tiers=%d, github=%s, captcha=%v, timstats=%s", info.host, info.port, info.node.portFull, info.amount, info.minutes, info.tiers, info.githubUser, info.captchaToken != "", info.node.timstats)
}
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			timstats: infos.envvars["ETH_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
