...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many TIMs to release per request? (default = %d)\n", infos.amount)
	...
	fmt.Println()
	if infos.node.timstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.timstats = w.readString() + ":" + w.conf.timstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.timstats)
		infos.node.timstats = w.readDefaultString(infos.node.timstats) + ":" + w.conf.timstats
	}
