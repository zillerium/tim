...
// config contains all the configurations needed by puppetim that should be saved
// between sessions.
type config struct {
	...
	timstats  string        // timstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save puppetim configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // tim services known to be running on servers
...
// converts it to an tim address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by timstats, which compares IPs textually, not by value.
