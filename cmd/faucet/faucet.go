...
// faucet is a Tim faucet backed by a light client.
package main
...
import (
	...
	"github.com/tim-coin/tim/accounts/keystore"
	...
	"github.com/tim-coin/tim/p2p/discv5"
	"github.com/tim-coin/tim/p2p/nat"
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the tim protocol")
	statsFlag   = flag.String("timstats", "", "timstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of Tims to pay out per user request")
	...
	logFlag = flag.Int("loglevel", 3, "Log level to use for tim and the faucet")
)

var (
	tim = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s Tims", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // tim address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an tim light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // tim protocol stack
	client *ethclient.Client   // Client connection to the tim chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the tim light client protocol
	...
	// Assemble the timstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.Lighttim
			ctx.Service(&serv)
			return timstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
...
// close terminates the tim connection and tears down the faucet.
...
// apiHandler handles requests for Tim grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		// Retrieve the tim address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/tim-coin/tim/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), tim)
...
func (f *faucet) loop() {
	...
	for {
		...
		case head := <-heads:
			...
			balance = new(big.Int).Div(balance, tim)
...
// the username, avatar URL and tim address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for tim addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No tim address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the tim address and profile URL.
...
// returning the username, avatar URL and tim address to fund on success.
