...
import (
	...
	"github.com/tim-coin/tim/common/bitutil"
...
func GetChtRoot(db timdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db timdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(ChtFrequency/ChtV1Frequency)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db timdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db timdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := timdb.NewTable(db, ChtTablePrefix)
	idb := timdb.NewTable(db, "chtIndex-")
...
func GetBloomTrieRoot(db timdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db timdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db timdb.Database, clientMode bool) *core.ChainIndexer {
	cdb := timdb.NewTable(db, BloomTrieTablePrefix)
	idb := timdb.NewTable(db, "bltIndex-")
