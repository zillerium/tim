...
#
# The cluster contains a bootnode, a timd node and multiple swarm nodes, with
...
# static timd configuration
timd_IP="192.168.33.3"
timd_RPC_PORT="8545"
timd_RPC_URL="http://${timd_IP}:${timd_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the timd and
# swarm nodes
start_bootnode() {
...
# start_timd_node starts a timd node with --datadir pointing at <base-dir>/timd
# and a single, unlocked account with password "timd"
start_timd_node() {
  local dir="${base_dir}/timd"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get timd account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${timd_IP}"
    --rpcport   "${timd_RPC_PORT}"
    --verbosity "6"
  )

  start_node "timd" "${timd_IP}" "$(which timd)" ${args[@]}
}
...
create_account() {
  ...
  timd --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
